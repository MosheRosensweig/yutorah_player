...
- **📝 Transcript & Study Aids (beta lecture pipeline)**:
  ...
  - Chapters/paragraphs click-to-seek; transcript auto-highlights the current paragraph during playback; quiz is two-phase (tap to select, Submit to lock) with numbered questions, stacked one-per-line options, green/red results in both themes, and ↩ hear-it links back to the audio moment. Audio tracks only; section hides when no transcript exists.
