...
- **Dark Mode / Light Mode Switcher**:
  ...
  - No saved choice (fresh browser, private window, or an iOS home-screen app — which Apple isolates with zero shared storage, so the theme cookie can't help) follows the device appearance via `prefers-color-scheme`; otherwise dark. An explicit saved choice (or `?theme=`/`?mode=` URL param) always wins. The OS-preference listener still applies only until the user picks explicitly.
  - **Theme is device-local, never contagious**: the choice is saved to `localStorage` plus a shared `yutorah_theme` cookie (so installed PWAs, which don't share `localStorage` with the browser tab, first-paint the same theme via server-side fallback). URL-supplied theme renders for that load only and is never saved; copied share links deliberately carry no theme — this fixed a bug where opening a dark-stamped link permanently flipped the opener's desktop to dark (the "faded blue" surprise).
...
- **📝 Transcript & Study Aids (beta lecture pipeline)**:
  ...
  - Three tabs under one toggle — 📋 Summary, 📝 Transcript (chapters + gap-grouped paragraphs), ❓ Quiz — parent collapses everything; only tabs with content render.
  - Chapters/paragraphs click-to-seek; transcript auto-highlights the current paragraph during playback and auto-scrolls it into view in the pane (pausing ~5s after manual scroll, never stealing page scroll); quiz is two-phase (tap to select, Submit to lock) with numbered questions, stacked one-per-line options, green/red results in both themes, and ↩ hear-it links back to the audio moment. Audio tracks only; section hides when no transcript exists.
  - Chapter ticks divide the scrubber at section boundaries (click a tick to jump) with a live `📑 <section>` title between the bar and the transport buttons, updating as it plays — independent of whether the transcript section is open.
  - Follow-scroll pins the active paragraph's first line to the transcript window top on every change (manual scroll suspends ~5s).
  - Enlarge reading mode (`⤢ Enlarge`): hides everything between the transport and the transcript (speed/copy/download/Daf row, series strip, metadata, description) and grows the transcript to ~65vh under the player, with a Transcript header + Collapse control; never leaks across tracks. Logged-in users get an account-menu "Enlarge transcript by default" checkbox (off by default).
