...
- **7-tap dev-mode toggle**: tapping the Hebrew date badge (or the calendar row in the login menu) 7 times toggles Dev Mode on/off symmetrically (2s idle reset; same toasts as typed flow). Typing `dev mode` / `exit dev mode` still works. The 3-tap pre-roll toggle stays exclusive to the holiday apple (its title now reads "Tap 3 times"), so the two gestures share no counter and never collide.
