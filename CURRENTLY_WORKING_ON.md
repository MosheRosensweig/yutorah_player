...
- **Phase 3 DONE** (`src/client/search-ui.js.txt` ~75KB, 29 fns, dev live): search bar/preview/filters/ranking/render carve. Scares handled: truncation bug in carver (fixed via paren-aware matcher + byte-fidelity proof for all 29); TEMPLATE-ENCODE lesson — carved bytes were template-encoded (`\\'`), standalone needs one decode level; proven by dev-vs-prod served-function diff (425/425 identical, only expected const-renames differ). Fixed latent honorific-strip regex (`s+`→`\s+`, unescaped dots) now visible in raw form. Reviews PASS.
- **Phase 4 DONE** (`transcript-pane.js.txt` 20 fns + `daf-guest.js.txt` 4 fns + frozen handoff protocol comment, dev `3ab0f655`): transcript system + daf glue carves. Backslash audit clean (transcript zero; daf single `\d` fixes latent prod `/^d{1,3}$/` bug as side effect). Review PASS (LOWs only). All 5 unit routes + home return 200 on dev.
- **Phase 5 DONE** (`src/client/theme-applier.js.txt`, 7 fns, dev `30dab6b2`): theme applier carve; `\u0590-\u05FF` decoded to literals (verified endpoints); dev-vs-prod served diff identical for key fns. Review PASS. Pre-existing LOW (not ours): `onHolidayThemeSelect` reads block-scoped `lockedThemesToA` → ReferenceError on non-yom_haatzmaut selects; fix separately.
- **Phases 2–6 plan note**: per approved plan in Research Q&A below (PlayerFacade → search engine/UI → daf+transcript guests → theme → OG pilot). Do NOT extract: playlists/queue/history/sync/auth split, article pipeline, series+grids split, `playShiurById`, SSR shell.
