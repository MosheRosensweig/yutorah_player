...
## 🎯 In Progress: Module Refactor Epic (branch `refactor/modules`, dev-only)
Goal: shareable units (player+mini, search, daf, transcript, theme) with ZERO functionality loss; easier maintenance. Conventions for this epic: one feature/fix per commit + test + review + dev deploy; notes updated every milestone (this file is the handoff log — an agent picking this up reads here first).
- **Phase 0 baseline**: 5/5 suites green (2026-09-20; basic needed one re-run — live-upstream flake, see incident).
- **Incident: upstream Solr page-hang** — `start=3` stalls indefinitely (query-dependent, intermittent); our fanout `Promise.all` hung forever (test 5 stalled 4+ min). Fix: `fetchWithTimeout()` (10s AbortController, portable, no `AbortSignal.timeout` dep) used in `fetchSolrSingle`; hang degrades to empty page per fanout-safety rule. Verified: suite green after.
- **Phase 1 (pure utils, single-source)**: escapeHtml ✅ DONE (see below) → formatShiurDate → formatTime/formatDuration → daf math (+ dedup `phonetic_engine.js:1454` twin). Pattern per util: ESM in `src/utils/` imported by worker (server) + classic bundle served at `/js/yt-utils.js` as `window.YTUtils` for the inline classic client script (inline `onclick` strings require classic scope — NO client ES modules). Bundle generated via fn `.toString()` (no drift); inline fallback kept for offline, pinned by behavioral equivalence test (server≡bundle≡fallback). Delete one copy per util, never fork a third.
- **Phase 1a DONE**: `src/utils/html.mjs` + `/js/yt-utils.js` route + shell script tag + SW precache + equivalence corpus test. Review PASS (follow-ups folded: equivalence test, incl. trailing-`;` eval fix). Baseline note: suite is flaky against live upstream (empty windows per query, varies run to run) — re-run, don't chase.
- **Phase 1b–d DONE** (dev `2582eea7`): `src/utils/dates.mjs` (4 fns), `src/utils/format.mjs` (2 fns), `src/utils/daf.mjs` (table + 5 fns); bundle is IIFE self-contained (review caught bare-sibling refs — fixed + standalone assert); fragment embeds `${DAF_CYCLE_DAYS}/${DAF_ANCHOR_UTC}`; served-HTML sourcing asserts (not just literals); dead imports removed; twin-follow-up marker placed. Reviews PASS.
- **Phase 2 DONE (PlayerChrome carve)**: `src/client/player-chrome.js.txt` (23 fns, logic-only) + pointer comments; emitted as own `<script>` (escaped) + served raw at `/js/player-chrome.js`; SW-precached; Node `--loader` for `.txt` in test script; moved-code asserts via `clientAllSrc()`. Review FAIL→fixed: unit top-level listeners ran before `const audio` (TDZ kills whole block) — registrations moved back to main after audio const; structural test (zero addEventListener in unit) prevents recurrence. Commit + deploy next.
- **Phases 2–6**: per approved plan in Research Q&A below (PlayerFacade → search engine/UI → daf+transcript guests → theme → OG pilot). Do NOT extract: playlists/queue/history/sync/auth split, article pipeline, series+grids split, `playShiurById`, SSR shell.
- **Phase 2 DONE** (`src/client/player-chrome.js.txt`, dev live): 23 transport/mini/session fns, pure code motion + raw emission. Reviews PASS.
- **Phase 3 DONE** (`src/client/search-ui.js.txt` ~75KB, 29 fns, dev live): search bar/preview/filters/ranking/render carve. Template-decode applied (standalone-correct escapes); dev-vs-prod served-function diff 425/425 identical. Latent honorific-regex bug fixed as found. Reviews PASS.
- **Phase 4 DONE** (`transcript-pane.js.txt` 20 fns + `daf-guest.js.txt` 4 fns, dev `3ab0f655`): transcript system + daf handoff glue carves; handoff protocol documented in unit header. Review PASS (LOWs only). All 5 unit routes + home return 200 on dev.
- **Phase 2 DONE** (`src/client/player-chrome.js.txt`, 23 fns, dev live): transport+mini+session carve, pure code motion. Review FAIL→fixed: unit top-level listeners ran before `const audio` (TDZ kills block) — registrations moved back to main; structural test (zero addEventListener in unit) prevents recurrence.
- **Phase 3 DONE** (`src/client/search-ui.js.txt` ~75KB, 29 fns, dev `010ed84e`): search bar/preview/filters/ranking/render carve. Scares handled: truncation bug in carver (fixed via paren-aware matcher + byte-fidelity proof for all 29); TEMPLATE-ENCODE lesson — carved bytes were template-encoded (`\\'`), standalone needs one decode level; proven by dev-vs-prod served-function diff (425/425 identical, only expected const-renames differ). Fixed latent honorific-strip regex (`s+`→`\s+`, unescaped dots) now visible in raw form. Reviews PASS.
...
- [x] **C1: Inline chapter dividers** — divider + bold header inside the transcript flow at each chapter boundary (metadata only: never highlight-targeted, never click-seek).
- [x] **C2: Chapter durations** — chapter buttons gain right-side `N min` (rounded, next-chapter fallback for the last) like the beta site.
- [x] **C3: Live chapter highlight + jump** — scrolling the transcript highlights the chapter in view; clicking a chapter scrolls to its first block (and seeks). N/A in enlarged (no headers shown). Light/dark correct. Review: PASS (3 MEDs folded in: shared table, duration fallback, snap refresh). *Status: ✅ dev-deployed (`9c7136e4`, HTTP 200).*
...
- Observed beta behavior: (1) the active caption highlights as audio crosses cue boundaries; (2) the transcript pane **auto-scrolls to keep the active cue visible**; (3) clicking any cue seeks the media to that timestamp.
...
- [x] **H1d: Strip-root-cause found + fixed (live bug)** — strips never appeared because single-backslash regex escapes in `seriesFamilyQuery` rendered as control chars through the server template literal (`\b` word-boundary arrived as backspace `\x08`, verified in served HTML), so part-marker stripping never ran and family queries over-matched. All 17 escapes doubled; served bytes re-verified (`\\b`). Also decoupled boot hydrate steps (one throw hid strip+transcript) and added `?diag=series` reporting. Regression test pins doubled escapes. *Status: ✅ dev-deployed (`5cebf0df`, HTTP 200).*
