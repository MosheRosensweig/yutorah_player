...
- **Single-User Duplicate Prevention**: A specific user cannot create two custom playlists with the identical name. Playlist names are validated upon creation and renaming.
...
### 2. Play Queue UX & Idempotent Modal Controls — `[COMPLETED ✅]`
- **Empty Queue Prompt Icon Fix**: Displays the authentic circular queue icon (`📋` / list icon).
- **Idempotent Single-Click Close**: Opening the queue is idempotent, and the `✕` close button unconditionally closes the queue on the very first click.

### 3. Playlists Tab Restructuring & Sticky Controls — `[COMPLETED ✅]`
- **Dual-Tab Hierarchy**: Inside the playlists view, segmented bar allows switching between `My Playlists` and `Public Playlists`.
- **Sticky / Frozen Top Controls**: Top system row is pinned (`History`, `Save for Later`, `Favorites`, `Play Queue`, and `+ New Playlist`).
- **Read-Only Saved Border Highlighting**: Subscribed read-only playlists display a distinct sky-blue border styling and badge (`📡 Subscribed · Live Sync`).

### 4. Playlist Sorting & Ordering — `[COMPLETED ✅]`
- **Sort Dropdown on Playlists**: Supports `Date Added`, `Shiur Date (Newest)`, `Shiur Date (Oldest)`, `Last Listened`, and `Manual / Drag-and-Drop` (with `▲` / `▼` buttons and drag handles).

### 5. Enhanced Playlist Creation Modal — `[COMPLETED ✅]`
- **Icon Selector**: 16 curated emojis with live preview in modal header.
- **Description Field**: Multiline description support.
- **Curated Searchable Tag Selector**: Structured tag selector with typeahead autocomplete from speakers, venues, and topics (max 5 tags).
- **Unique Name Validation**: Real-time validation preventing duplicate playlist names.

### 6. Public Playlists Browser: Dynamic Keyword Filters — `[COMPLETED ✅]`
- Filter dropdowns and datalists populated dynamically with teachers, venues, and topics, plus keyword search and scope toggles.

### 7. Public Playlist Cards & Preview Mode — `[COMPLETED ✅]`
- **No Self-Saving**: Users cannot see a "Save" button on public playlists that they themselves authored (renders `👤 Your Playlist` badge).
- **Preview Shiurim Modal / Drawer**: Displays lecture title, speaker, duration, and formatted date given/uploaded.
- **Save Choice Modal**: Clean choice between `📡 Subscribe (Live Sync)` and `📋 Make Editable Copy`.

### 8. Add-to-Playlist Multi-Select Popup — `[COMPLETED ✅]`
- **Live Counter Tick**: Checking a playlist increments count with green badge (`+1`), unchecking decrements with red badge (`-1`).
- **Explicit Save Commitment**: Changes are staged until clicking `💾 Save Changes` (or `Cancel` to discard).
...
| **9** | **YUTorah.org Feature Parity Audit (Dual-Agent) & Gap Roadmap** *(see §10)* | Two independent agents comprehensively audited live `yutorah.org` (homepage rotating slideshow, series carousel, search facets, browse directories, lecture detail, accounts, RSS/podcasts) against our player and converged on a unified gap list (§10.1–10.3) with implementation approaches and per-feature acceptance tests. Builds in value/effort order; read-paths reuse open-CORS `api.yutorah.org` + `/sidebar/*` via the Worker edge cache, never scraping Turnstile-gated HTML. | **Medium (audit done; build phased S–L per item)** | **VERY HIGH**<br>★★★★★ |
...
### 5.4 Fuzzy "Did You Mean" (Levenshtein) Across Teachers, Topics & Venues
When a search returns zero results (or the user pauses mid-typing where results would normally appear), the engine falls back to edit-distance matching against known entity indexes (teacher names, topic/subcategory names, venue names, plus the Tier 1 synset lexicon):
- **Example**: typing `weiderblank` (intended `wiederblank`) pauses → dropdown suggests the closest matches; hitting Search with no direct hits renders a `Did you mean: wiederblank?` row with zero, one, or more ranked candidates.
- **Algorithm**: normalized Damerau-Levenshtein (insert/delete/substitute/transpose) over lowercased, honorific-stripped candidates; rank by distance then by popularity (`numFound`/lecture count); threshold gate (e.g. distance ≤ 2, or ≤ 25% of term length) so garbage input yields zero suggestions rather than noise. Runs against prebuilt edge indexes (teachers/venues/subcategories from the `facet=true` endpoint, nightly KV refresh), <5ms, no upstream calls on keystroke.
- **Two surfaces, same engine**: (a) **as-you-type**: on input-pause (~200–300ms debounce), suggestion chips appear under the search box; (b) **after Search**: a `Did you mean …?` strip above the results when `numFound == 0` (or below results when 1+ weak hits exist).
- **Click behavior**: clicking a suggestion puts that exact term back into the search box (and re-runs the search), preserving any active filters; keyboard navigable (↑/↓ + Enter/Esc).
...
1. **Recent Results Sub-Section (with its own "Load More")**:
...
2. **Relevance / All Matching Sub-Section (with its own separate "Load More")**:
   ...
   - Preserves its own independent continuous pagination via the existing `"🔽 Load More Results"` button, seamlessly fetching additional pages into the relevance section.
   - The two "Load More" buttons operate independently — expanding the Recent section does not affect the Relevance section and vice versa.
...
## 10. YUTorah.org Feature Parity Audit — Dual-Agent Findings, Build Plan & Acceptance Tests

> Method (2026-09-09): two independent subagents (A and B) audited live `yutorah.org` via different paths (A: saved `actual webpages downloaded/` snapshots + live `api.yutorah.org` probes + worker keyword counts; B: search-engine cache excerpts + saved HTML + live `/homepage/details` JSON) because direct `WebFetch https://www.yutorah.org/` returns `HTTP 403 cf-mitigated: challenge` (Turnstile). Both diffed against `index.html`, `FEATURES.md`, `YUTORAH_API_SPECIFICATION.md`, and `src/worker.js`. What follows is their converged output. Already-have items (collection tabs, timely/Parsha/Daf/Mishna/Nach strings + Hebrew date, Solr search + filters modal, bios/venues, mini-player, speed/skip, failover, MP3 download, `?t=` links, dark mode, recommendations, sponsorship banner + pre-roll, OG tags, article proxy) are excluded.

### 10.1 What yutorah.org has (observed upstream)

1. **Homepage rotating slideshow (hero)** — container `slideshow gallery-js-ready autorotation-active` (auto-rotating fading hero, JS gallery). Live `GET /homepage/details → slideshow[4]`; each `{name, description, imageURL, imageThumbnailURL, urlTitle, targetURL}`. Main image 682×369 (+retina variants), thumbs 201×84. Content is seasonal/campaign-driven (e.g. Selichot / Zman opening / Rosh Hashana shiurim / Torah To-Go issue / Support-Donate). Clicks go through tracked redirector `/Slideshow/Go?id=N&dest=…` to lectures, pre-filtered searches (`/search/?s=…&sort=1`, `/search/?category=0,…`), `/daf`, `/daily-shiur/`, `/togo/*`, or external GiveCampus donate.
2. **Homepage series carousel (filmstrip)** — live `carousel[20]`, banners 268×110 from `/_files/adCarousel/*.jpg`, in `#carouselToReplace` (wide filmstrip, JS-scrolled, ~4–5 visible). Almost all items are **series promos** (Rabbi X's Y-series). Clicks go through `/Carousel/Go?id=N&dest=/search/?teacher=X&collection=Y|category=0,Z` — curator-built entry points into pre-filtered teacher+series/category searches.
3. **Homepage tab system (7 tabs)** — `data-name=`: `editors-picks`, `featured-series`, `recently-uploaded`, `recently-viewed`, `parsha-shiurim`, `daily-shiurim`, `trending-keywords`. Backed by `editorsPicks[10]`, `featuredSeries[10]` (`{seriesID, name, description, imageURL, numShiurim}`), `recentlyUploaded[10]`, `recentlyViewed[]` (24h window, often `[]`), `parshaShiurim[]` (seasonal, `[]` off-season), `dailyShiurim[10]`. Per-column AJAX `More` buttons + login-gated gear (`Save search / Remove saved search / Login to use this feature`).
4. **Search UX** — `<form action="https://www.yutorah.org/Search">` with jQuery-UI autocomplete and live-count placeholder (`Search through 440,126 shiurim…`). Facets on results: `Type: All|Audio|Video|Text`, `Duration:`, `Date uploaded` (`?dtu=1` Today / `?dtu=2` Yesterday), `Language:`, `Sort` (`&sort=1` = date/recency). Series view adds tabs `General|Featured|Venues|Categories|Teachers|Collections|Favorites` with note `Filters do not affect the collections tab`. Result cards: title, Speaker, Date, Duration, Download / Play Later (Queue Up) / Play Now (Read Later for text). Queue chrome: `In progress (n) / Upcoming (n) / Past (n)`, `My shiurim`, `Clear`, `Bulk download with iTunes / Bulk download list`, `Save as Collection`, `Check/Uncheck All → Add to Queue`, `Save search`. Pagination is per-column AJAX `More`, totals shown (`Shiurim & Articles (42079)`).
5. **Browse directories** — hamburger mega-menu: Categories tree (Gemara → Bavli → …; 601 subcategories via facet API), Teachers (3,254), Venues (405), Series (49), Publications (43), Volumes (419), Collections (13,289). Directory pages `/teachers/{Name}` (bio + `Shiurim & Articles (n)` + `Recently Added` + `Top Lectures`), `/categories/…`, `/series/Daily-Shiur/`, `/venues/…`, `/publications?publicationid=229` (To-Go). Sidebar landing APIs: `/teachers|venues|categories|series/sidebar/{id}` (bio, photos, `landingPageURL`, `searchURL`).
6. **Daily-study trackers** — `timelyData {parshaStr/URL + israelParshaStr/URL, dafStr, mishnaYomiStr+ID, nachYomiStr+ID}` + Hebrew date string + Zmanim widget. Dedicated `/daf` tracker page (`Today's Daf`, In-progress/Upcoming/Past). Publications/To-Go journal system: issues as lectures with PDFs on `download.yutorah.org`, `Editor To-Go`, `Read Later`, `/togo/*` landings, per-publication Podcast + RSS.
7. **Lecture detail** — title, Speaker + `Ask speaker`, Venue (linked), Date, Length, `Downloads:` + `Views:` + `Comments (n)`, `Collections:` membership, description, `Materials & Sources / References` (`Show All / Show Less`), topic lines, `Statistics:`, `About this Shiur` tabs, `More from this: SPEAKER|SERIES|CATEGORY`, `Comments + Leave a Comment` (login-gated), `Share / Embed / Facebook / Twitter / Email / Print`, jPlayer (`play/seek/volume/1.0x speed`), `vtt-transcript.js` present, `mediaTypeCategory: audio|video|text` (MP3/MP4/`shiurText`), `shiurTags/Materials`, OG + `schema.org/Event`, some downloads login-gated (`download disable / Login to use this feature`), `errors@yutorah.org` + `tracker/shiurDownload|Print.cfm` pixels.
8. **Accounts & subscriptions** — username/password (`keepMeLoggedIn`), SignUp, Google + Facebook OAuth, `POST /Account/UserFavorites {teacher|series|location|collection}`, `GET|POST /Queue/queue {queue|history|articles}`, saved-search list, bulk-download list, Subscription Center (newsletter, To-Go packets), per-speaker/series RSS (`/rss/RssAudioOnly/series/{id}`, RSS 2.0 + iTunes, verified live) + `Subscribe to Podcast`, Contact/Feedback, Terms/Privacy. Native iOS/Android apps (offline downloads, speed, favorites, resume, CarPlay) — noted, not cloned.

### 10.2 Unified gap roadmap (build order by value/effort; S ≤1d, M 2–4d, L 5d+)

| # | Gap → our build | Approach (Worker edge + client; reuse open-CORS `api.yutorah.org` + `/sidebar/*`, KV/D1 cache; never scrape gated HTML) | Effort |
|---|---|---|---|
| P1 | Hero slideshow + series carousel | Worker `GET /api/home/slideshow\|carousel` from cached `/homepage/details` (1h TTL). Client `<HeroSlideshow>` (responsive 682×369 art, caption + CTA, CSS fade, ~6s autoplay, dots + arrows, pause-on-hover; map `targetURL` to internal lecture/search route, external GiveCampus opens new tab; hide when `[]`) + snap-scroll filmstrip of 268×110 thumbs with arrows, resolving `/Carousel/Go` dest queries into filter state. | M |
| P2 | Missing homepage tabs (Parsha / Recently Viewed / Trending / Featured Series) + total count | Render `parshaShiurim` (hide when `[]`), Recently-Viewed from local history pre-auth → cloud post-auth, Featured Series cards (art + `numShiurim` → `/series/{id}` detail via `/series/sidebar/{id}` + `search?seriesId=`), header `totalShiurCount` social proof (5-min cache). Trending via `shiurTags` frequency spike first. | S (M if trending needs new upstream) |
| P3 | Lecture detail enrichment + share row | Render views/downloads/comments counts, Collections chips, collapsible Materials/Sources (`Show All/Less`), topic lines; share row = `navigator.share` + WhatsApp/FB/X/Email intents + copy-link-@-time (exists) + embed `<iframe>` modal; `mailto:errors@yutorah.org`, ask-speaker link-out, print stylesheet; JSON-LD `Event/AudioObject`. | S |
| P4 | Ship search facets + sort (incl. §7 Load-More-Recent interplay) | Wire local Filters modal: push `teacherId/subCategoryId/locationId/seriesId/searchTerm` to Solr; post-filter duration/year + `mediaType` Audio/Video/Text + `language` EN/HE + `dtu` date-uploaded via `shiurdate` ranges; sort control Relevance/Newest/Oldest/Most-viewed (spike exact upstream `sort=` values); keep 30-item page invariant + independent Recent/Relevance Load-Mores from §7. | M |
| P5 | Autocomplete + fuzzy Did-You-Mean + browse directories + Daf tracker | `GET /api/suggest?q=` (150ms debounce dropdown, keyboard nav; teacher index + `autocomplete_data.json`, nightly KV; placeholder count from `totalShiurCount`); fuzzy fallback per §5.4 (Damerau-Levenshtein over teachers/topics/venues, as-you-type chips + post-search `Did you mean …?` strip, click fills search box); `GET /api/browse/{teachers,venues,series,publications,collections,subcategories}` from `facet=true&rows=0` (24h KV, chunked) → A–Z + counts + detail via sidebar endpoints, category mega-menu from `subcategories` grouped by `categoryName`; `/daf` view from `homepage/timely` + subcategory searches. | M |
| P6 | Queue tabs + bulk download + video/articles | Queue drawer In-progress/Upcoming/Past reusing §8 playlist store (cloud sync later); checkbox mode → `m3u`/OPML export + sequential downloads (`Save as Collection` → local playlist); `<video>` for `mediaTypeCategory=video` (range intact, PiP/docked); article reader renders `shiurText` + Read-Later. | M |
| P7 | Accounts, favorites, saved searches, comments, RSS-out | Follow §1 (own Google OAuth PKCE + D1; do NOT proxy `.yutorah.org` cookies): D1 `favorites` (teacher/series/location/collection) + `saved_searches` + own per-shiur comments; interim localStorage saved-searches; per-speaker/series RSS buttons emitting native `/rss/RssAudioOnly/…` + `<link rel=alternate>`; newsletters = OPML/`mailto:` links (no write to their Subscription Center). | L (S slices: local favs, RSS buttons, footer links) |

### 10.3 Acceptance tests (done = each line verifiably passes)

- P1: slideshow renders 4 slides with art/caption/CTA, auto-advances ~6s, pauses on hover, dots + arrows work, each click lands on correct lecture/search/donate; carousel shows 20 thumbs, arrows scroll, each resolves to the right pre-filtered search; both hidden cleanly when arrays are `[]`.
- P2: Parsha tab hides off-season and shows in-season; Recently-Viewed matches local history; Featured Series cards show art + counts and open detail grids; header count equals `totalShiurCount`.
- P3: detail shows views/downloads/comments/collections/materials with Show All/Less; share sheet + each intent + embed snippet produce working links; `mailto:errors@` prefilled; JSON-LD validates.
- P4: each facet (type/duration/date/language/sort) narrows results and survives Load-More in both §7 sub-sections; 30-item pages hold; combined filters equal upstream `numFound` within page tolerance.
- P5: suggestions appear <300ms with keyboard selection; typing `weiderblank` suggests `wiederblank`, zero-result searches show a `Did you mean …?` strip (0/1/many) and clicking a candidate fills the search box and re-runs; directories list counts and open filtered searches; bios/venues load via sidebar endpoints; `/daf` shows today + progress groupings.
- P6: queue tabs reflect played/saved/upcoming states; bulk export downloads or enqueues all checked items; MP4 plays with speed/seek; articles render full text with Read-Later.
- P7: login persists 1yr session, history/favorites/saved-searches sync across devices, logout + GDPR export/erasure work; RSS buttons return valid feed XML; comments post/list under auth.
