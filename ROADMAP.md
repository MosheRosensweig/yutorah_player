...
| P5 | Autocomplete + browse directories + Daf tracker | `GET /api/suggest?q=` (150ms debounce dropdown, keyboard nav; teacher index + `autocomplete_data.json`, nightly KV; placeholder count from `totalShiurCount`); `GET /api/browse/{teachers,venues,series,publications,collections,subcategories}` from `facet=true&rows=0` (24h KV, chunked) → A–Z + counts + detail via sidebar endpoints, category mega-menu from `subcategories` grouped by `categoryName`; `/daf` view from `homepage/timely` + subcategory searches. | M |
...
- P5: suggestions appear <300ms with keyboard selection; directories list counts and open filtered searches; bios/venues load via sidebar endpoints; `/daf` shows today + progress groupings.
